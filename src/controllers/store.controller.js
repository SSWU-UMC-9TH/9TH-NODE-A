...
export const handleAddReview = async (req, res, next) => {
  ...
  #swagger.responses[400] = {
    description: "storeId, rating이 유효하지 않은 경우",
    content: {
      "application/json": {
        schema: {
          type: "object",
          properties: {
            resultType: { type: "string", example: "FAIL" },
            error: { type: "object", example: { code: "B400", message: "유효한 storeId, rating이 필요합니다." }},
            success: { type: "object", nullable: true, example: null }
          }
        }
      }
...
*/
  ...
  const { rating, content, missionId } = req.body;
  const userId = req.user.id;

  if (Number.isNaN(storeId) || rating == null) {
    throw new BadRequestError("유효한 storeId, rating이 필요합니다.");
  }
  try {
    const reviewId = await addReviewToStore(storeId, {
      userId,
      missionId,
      rating,
      content,
